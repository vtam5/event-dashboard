// frontend/src/utils/eventForm.js
...
// Map legacy isPublished → status if needed (back-compat only)
...
export function toPayload(values) {
  const status = String(values.status || 'private').toLowerCase();
  return {
    name: values.name,
    ...
    // send status (we no longer send isPublished)
    status,

    allowResponseEdit: !!values.allowResponseEdit,

    capacityLimit:
      values.capacityLimit === '' || values.capacityLimit == null
        ? null
        : Number(values.capacityLimit),
